...
import { mockProjects, projectCategories } from "@/lib/mock-data";
import { containerFast, fadeUpSmall } from "@/lib/animations";

const Projects = () => {
  const [activeCategory, setActiveCategory] = useState<string>("all");
  const filtered = activeCategory === "all" ? mockProjects : mockProjects.filter((p) => p.category === activeCategory);
  ...
  return (
    <div className="py-10 md:py-14">
      <div className="container">
        ...
        {filtered.length === 0 && <div className="py-20 text-center text-sm text-muted-foreground">هیچ پڕۆژەیەک نەدۆزرایەوە</div>}
      </div>
    </div>
  );
};
